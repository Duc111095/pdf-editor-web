...
import com.ducnh.excellentPdf.controller.api.pipeline.UserServiceInterface;
import com.ducnh.excellentPdf.model.ApplicationProperties;
import com.ducnh.excellentPdf.model.PdfMetadata;
...
public class PdfMetadataService {
	...
	private void setNewDocumentMetadata(PDDocument pdf, PdfMetadata pdfMetadata) {
		...
		if (applicationProperties.getEnterpriseEdition().getCustomMetadata().isAutoUpdateMetadata() && runningEE) {
			
			creator = applicationProperties.getEnterpriseEdition().getCustomMetadata().getCreator();
			pdf.getDocumentInformation().setProducer(creator);
		}
