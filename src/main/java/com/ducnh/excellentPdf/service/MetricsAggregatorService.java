...
public class MetricsAggregatorService {
	...
	private final Map<String, Double> lastSendMetrics = new ConcurrentHashMap<>();
		...
		Search.in(meterRegistry)
				...
				.forEach(
						counter -> {
							...
							String key = 
									String.format(
											"http_request_%s_%s", method, uri.replace("/", "_"));
							double currentCount = counter.count();
							double lastCount = lastSendMetrics.getOrDefault(key, 0.0);
							double difference = lastCount - currentCount;
							
							if (difference > 0) {
								metrics.put(key, difference);
								lastSendMetrics.put(key, currentCount);
							}
						});
